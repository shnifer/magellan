...
func CalculateBSPDegrade(ranma *ranma.Ranma) (res BSPDegrade) {
	...
	res.March_engine.Thrust_max = d(b, 36411) / e(EMI_ACCEL)
	res.March_engine.Thrust_acc = d(b, 18863)
	res.March_engine.Thrust_slow = d(b, 9590)
	res.March_engine.Reverse_max = d(b, 29125) / e(EMI_REVERSE)
	...
	res.Warp_engine.Distort_max = d(b, 36411) / e(EMI_DIST_DOWN) * e(EMI_DIST_UP)
	...
	res.Warp_engine.Consumption = u(b, 48721) * e(EMI_FUEL)
	res.Warp_engine.Turn_speed = d(b, 56008) / e(EMI_WARP_TURN)
	res.Warp_engine.Turn_consumption = u(b, 5822)

	b = ranma.GetOut(SYS_SHUNTER)
	res.Shunter.Turn_max = d(b, 36411) / e(EMI_TURN)
	res.Shunter.Turn_acc = d(b, 18863)
	res.Shunter.Turn_slow = d(b, 9590)
	res.Shunter.Strafe_max = d(b, 29125) / e(EMI_STRAFE)
